...
7. 必要に応じて編集可能な Excel ブックを生成する
- ユーザーが「あとで容量や転送量を変えて再計算したい」「Excel で渡したい」と求めた場合は、Markdown 出力に加えて `.xlsx` を生成する。
- ブックは最低でも次のシートに分ける:
  - `入力値`: 容量、転送量、台数、WAF 前提、ALB LCU、Route 53 前提、USD/JPY
  - `単価`: AWS 公式ソースから取得した USD 単価
  - `見積`: 各リソースの数量、単価、月額 USD、月額 JPY を Excel 数式で計算
  - `使い方`: どのセルを編集すればよいかを日本語で説明
- ブック生成ルール:
  - 表記はユーザー指定がない限り日本語にする。
  - `入力値` シートの編集セルと `単価` シートの編集セルを明確に分離する。
  - `見積` シートでは、数量・単価・小計・総計をすべて Excel 数式で持たせる。
  - `ALB` は可能な限り `時間課金` と `LCU 課金` を分離する。
  - `Route 53` は少なくとも `公開ホストゾーン` と `標準クエリ` を別行で持たせる。
  - `WAF` は少なくとも `Web ACL`、`マネージドルール`、`リクエスト` を別行で持たせる。
  - 罫線、見出し色、列幅、先頭行固定などの最低限の可読性改善を入れる。
  - ブックを開いたときに再計算されるように `calcPr` を設定する。
- 実装方針:
  - 使えるライブラリがない場合でも、Open XML 形式の最小構成を書き出して `.xlsx` を生成してよい。
  - 生成スクリプトも保存し、次回以降の再利用を容易にする。
  - 最終回答では、生成した `.xlsx` の保存先と、ユーザーが編集すべきシート/列を短く案内する。
...
- 参照URLがすべて公式ソースか。
- Excel を生成した場合、編集セル・数式セル・除外項目がブック内で判別できるか。
- Excel を生成した場合、ユーザーが容量や転送量を変えて再計算できる構造になっているか。
