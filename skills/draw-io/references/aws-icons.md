...
## 5.5 表示のトラブルシューティング（実作業で得た知見）

アイコンが draw.io で表示されない場合、以下を順に試す。

### 正しい resIcon 名の確認

- **Amazon S3**: `resIcon=mxgraph.aws4.s3` を使用する。`s3_bucket` や `simple_storage_service` は環境によっては表示されない。本カタログの「3.3. Storage」では **Amazon S3 → mxgraph.aws4.s3** を採用する。
- 他のサービスも、スクリプトやカタログで複数候補がある場合は、**同じプロジェクト内で表示できている .drawio の style を grep で確認**し、その `resIcon` を優先する。

### 既存 .drawio を参照する

1. プロジェクト内の他の `.drawio` で、目的のアイコン（EC2・RDS・S3 など）が正しく表示されているファイルを探す。
2. そのファイル内で該当セルの `style` を検索する（例: `grep -n "resourceIcon\|resIcon\|shape=mxgraph.aws4" 参照ファイル.drawio`）。
3. 表示できているセルと同じ `shape=` および `resIcon=`（または専用シェイプ名）を、編集対象の図のセルにコピーする。
4. **mxfile の version と agent を参照ファイルに揃える**と、同じ draw.io ライブラリが読み込まれ、表示が揃いやすい。
   - 例: 参照ファイルが `version="29.3.8"` かつ `agent="Mozilla/5.0 ... Chrome/144.0.0.0 ..."` なら、プログラム生成した図の mxfile も同じにするとよい。

### 専用シェイプと resourceIcon の使い分け

- ALB / NLB / Internet / NAT Gateway / VPC Endpoints / Peering などは、**専用シェイプ**（`shape=mxgraph.aws4.application_load_balancer` など）で指定されていることが多い。参照 .drawio では `resourceIcon` ではなく `shape=mxgraph.aws4.xxx` のみの形式を確認する。
- S3 は **resourceIcon** で `resIcon=mxgraph.aws4.s3` を使う形式で表示できる。

### エッジの接続

- 線の接続先は **セル ID で指定**する（`target="セルID"`, `source="セルID"`）。座標のみの `mxPoint` に頼ると、セルを差し替えたあと線が正しく繋がらない。
- セルを差し替えた場合は、エッジの `target` / `source` を新しいセルの `id` に更新する。
