...
python ~/.claude/skills/draw-io/scripts/find_aws_icon.py s3
```

Uses latest `mxgraph.aws4.*` icon set. If an icon does not display (e.g. S3), use `resIcon=mxgraph.aws4.s3` (not `s3_bucket`) and align the diagram’s mxfile `version`/`agent` with a working `.drawio` in the project; see `references/aws-icons.md` §5.5.
